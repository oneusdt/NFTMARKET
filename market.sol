...
contract Market is Ownable, ReentrancyGuard, ERC1155Receiver {
  ...
  constructor(address payable _king,uint256 _commissionPercentage, uint256 _minimumCommission, uint256 _minimumBidSize, uint256 _minimumAuctionLiveness) {
      king = _king;
      commissionPercentage = _commissionPercentage;//佣金百分比 10000000000000000
      minimumCommission = _minimumCommission;//最低佣金 1000000000000000
      minimumBidSize = _minimumBidSize;//最小投标尺寸 100000000000000
      minimumAuctionLiveness = _minimumAuctionLiveness;//最低拍卖活跃度 600 
  }
 ...
 event UserCredited(
   ...
   */
  function settleAuction(uint256 auctionId) public openAuction(auctionId)  nonReentrant{
  ...
  function placeBid(uint256 auctionId, uint256 bidPrice) public payable openAuction(auctionId) nonExpiredAuction(auctionId) nonReentrant{
    ...
    function close()  external ownerOf{
